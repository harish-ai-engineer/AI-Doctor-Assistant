  ...
  return (
    <div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        ...
        <ChartCard title="Daily model cost" description="OpenAI spend attributed by Langfuse"><AreaChart data={usageData}><defs><linearGradient id="cost" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#38bdf8" stopOpacity={.4} /><stop offset="1" stopColor="#38bdf8" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="4 4" vertical={false} stroke="hsl(var(--border))" /><XAxis dataKey="day" axisLine={false} tickLine={false} /><YAxis axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12 }} /><Area type="monotone" dataKey="cost" stroke="#38bdf8" strokeWidth={2.5} fill="url(#cost)" /></AreaChart></ChartCard>
      </div>
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        ...
        <Card className="p-6"><h2 className="font-bold">Service health</h2><p className="mb-6 text-xs text-muted-foreground">Application dependencies</p><div className="space-y-5">{[["FastAPI", "99.99%", Server, 99.99], ["OpenAI GPT-4o", "99.92%", Cpu, 99.92], ["Langfuse", "100%", Activity, 100], ["ChromaDB", "99.97%", Server, 99.97]].map(([name, value, Icon, health]) => { const IconComponent = Icon as typeof Server; return <div key={String(name)}><div className="mb-2 flex items-center justify-between text-sm"><span className="flex items-center gap-2"><IconComponent className="size-4 text-primary" />{String(name)}</span><b>{String(value)}</b></div><Progress value={Number(health)} /></div>; })}</div></Card>
      </div>
    </div>
  );
}
